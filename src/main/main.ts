...
import ScratchWords from './scratch-words';
...
ipcMain.on('set-size', async (event, arg) => {
  const msgTemplate = (pingPong: string) => `IPC: ${pingPong}`;
  console.log(msgTemplate(arg));
  mainWindow?.setSize(arg[0], arg[1]);
  event.reply('set-size', msgTemplate('set-size-callback'));
});
